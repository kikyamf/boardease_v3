<?php
// Test script to verify that chat list duplicates are finally fixed
require_once 'db_connection.php';

$user_id = 29; // Test with user 29 (owner)

echo "=== TESTING FINAL DUPLICATE FIX ===\n";
...
try {
    ...
    // Test the individual chats query with the new approach
    if ($current_user['user_role'] === 'BH Owner') {
        echo "=== OWNER SIDE QUERY WITH ROW NUMBERING ===\n";
        $stmt = $db->prepare("
            SELECT 
                other_user_id,
                first_name,
            ...
            FROM (
                SELECT 
                    ...
                    @row_number := CASE 
                        WHEN @prev_user = CASE WHEN m.sender_id = ? THEN m.receiver_id ELSE m.sender_id END 
                        THEN @row_number + 1 
                        ELSE 1 
                    END as rn,
                    @prev_user := CASE WHEN m.sender_id = ? THEN m.receiver_id ELSE m.sender_id END
                FROM messages m
                JOIN users u ON (
                    CASE 
                        WHEN m.sender_id = ? THEN m.receiver_id
                ...
                CROSS JOIN (SELECT @row_number := 0, @prev_user := '') r
                ...
                ORDER BY other_user_id, m.msg_timestamp DESC
            ) ranked_messages
            WHERE rn = 1
