...
try {
    ...
    // Query for pending bookings (Stage 1 or Stage 2)
    // Stage 1: booking_status = 'Pending'
    // Stage 2: booking_status = 'Approved' AND has a 'Pending' payment
    $sql = "
        SELECT 
            b.booking_id,
            b.room_id,
        ...
        AND (
            ...
            OR (
                ...
                AND EXISTS (SELECT 1 FROM payments WHERE booking_id = b.booking_id AND payment_status = 'Pending')
            )
        )
    ...
    foreach ($results as $row) {
        ...
        $pendingBookings[] = [
            ...
            'total_months_for_booking' => $totalPeriods, // Assuming periods map to months for now
