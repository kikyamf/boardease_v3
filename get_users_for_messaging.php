...
try {
    ...
    if ($current_user['user_role'] === 'BH Owner') {
        ...
        // Get boarders who are actively staying in the owner's boarding houses
        $stmt = $db->prepare("
            SELECT 
                u.user_id,
        ...
        // If no active boarders found, return empty array
        if (empty($users)) {
            $users = [];
        }
        ...
        // First, find which boarding house this boarder is staying in through active_boarders table
        $stmt = $db->prepare("
            SELECT 
        ...
        if ($boarder_bh) {
            ...
            // Get other boarders from the same boarding house through active_boarders table
            $stmt = $db->prepare("
                SELECT 
                    u.user_id,
                    CONCAT(r.first_name, ' ', r.last_name) as full_name,
