<?php
// Debug what your Android app is actually calling - FIXED VERSION
...
try {
    ...
    echo "=== DEBUGGING YOUR ANDROID APP'S CHAT LIST (FIXED) ===\n\n";
    
    // 1. First, let's check the actual database schema
    ...
    $stmt = $db->prepare("DESCRIBE registrations");
    $stmt->execute();
    $reg_columns = $stmt->fetchAll();
    echo "\nRegistration table columns:\n";
    foreach ($reg_columns as $col) {
        echo "- {$col['Field']} ({$col['Type']})\n";
    }
    
    // 2. Check all users with correct column names
    echo "\n2. ALL USERS AND THEIR RECENT MESSAGES:\n";
    $stmt = $db->prepare("
        SELECT 
            u.user_id,
            r.f_name,
    ...
    // 4. Test different user IDs to see which one matches your app
    echo "\n4. TESTING DIFFERENT USER IDs TO MATCH YOUR APP:\n";
