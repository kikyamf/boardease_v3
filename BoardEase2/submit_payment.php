...
try {
    ...
    try {
        ...
        try {
            ...
            $notifTitle = "Payment Pending Approval";
            $notifMessage = $boarderName . " has submitted a payment of ₱" . number_format($totalAmount, 2) . " for " . $roomName . ". Please review and approve.";
            
            // Check for duplicate notification (prevent duplicates)
            $checkDuplicateSql = "SELECT notif_id FROM notifications 
                                 WHERE user_id = :owner_id 
                                 AND notif_type = 'payment' 
                                 AND notif_title = :title
                                 AND notif_message LIKE :message_pattern
                                 AND notif_created_at >= DATE_SUB(NOW(), INTERVAL 10 SECOND)
                                 LIMIT 1";
            $checkDuplicateStmt = $pdo->prepare($checkDuplicateSql);
            $messagePattern = "%₱" . number_format($totalAmount, 2) . "%";
            ...
            if (!$duplicate) {
                $insertNotifSql = "INSERT INTO notifications (user_id, notif_title, notif_message, notif_type, notif_status) 
                                  VALUES (:owner_id, :title, :message, 'payment', 'unread')";
                ...
                // Try to send FCM notification if available
                ...
                if (file_exists($fcmFile)) {
                    ...
                    if (function_exists('sendFCMNotification')) {
                        sendFCMNotification($ownerId, $notifTitle, $notifMessage, [
                            'type' => 'payment',
                            'payment_id' => $paymentId,
                            'booking_id' => $bookingId,
                            'status' => 'Pending'
                ...
                error_log("Duplicate notification prevented for owner (user_id: $ownerId) - payment_id: $paymentId");
            }
        } catch (Exception $e) {
