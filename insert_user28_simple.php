<?php
// Insert user_id = 28 as boarder in bh_id = 85 (without room_id for now)
error_reporting(E_ALL);
ini_set('display_errors', 1);

require_once 'db_helper.php';

echo "Inserting user_id = 28 as boarder in bh_id = 85...\n\n";
...
try {
    ...
    // Insert new entry (without room_id for now)
    $stmt = $db->prepare("
        INSERT INTO active_boarders (user_id, boarding_house_id, status, created_at, updated_at) 
        VALUES (?, ?, 'Active', NOW(), NOW())
    ");
    $stmt->execute([28, 85]);
    echo "✅ Inserted user 28 as boarder in boarding house 85\n";
